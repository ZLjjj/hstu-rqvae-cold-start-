...
class CandidateIndex(torch.nn.Module):
    ...
    def get_top_k_outputs(
        ...
        # Keep all retrieval operands on the same device as query embeddings.
        query_device = query_embeddings.device
        item_embeddings_t = self._embeddings_t
        item_ids = self._ids
        if item_embeddings_t is not None and item_embeddings_t.device != query_device:
            item_embeddings_t = item_embeddings_t.to(query_device)
        if item_ids.device != query_device:
            item_ids = item_ids.to(query_device)
        if invalid_ids is not None and invalid_ids.device != query_device:
            invalid_ids = invalid_ids.to(query_device)

        k_prime = min(k + max_num_invalid_ids, self.num_objects)
        top_k_prime_scores, top_k_prime_ids = self._top_k_module(
            query_embeddings=query_embeddings,
            item_embeddings_t=item_embeddings_t,
            item_ids=item_ids,
            k=k_prime,
            sorted=True,
        )
