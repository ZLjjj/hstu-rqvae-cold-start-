...
For the shared `NewsRecommend` conda runtime used by this project, install:
...
## Cold-start Experiments (RQ-VAE bridge)

Run all commands inside the `NewsRecommend` conda env:
...
Route A (semantic embedding replacement):

```bash
make train experiment=ml-1m-hstu-semantic-cold
```

Dense baseline (MiniLM dense vector + projection):

```bash
make train experiment=ml-1m-hstu-dense-cold
```

Route B (hierarchical token autoregressive retrieval + beam search + exact补全):

```bash
make train experiment=ml-1m-hstu-token-cold
```

The cold-start protocol is strict item cold-start (`cold_item_ratio=0.1`, fixed seed), with extra metrics automatically reported as:

- `cold_hr@k`, `cold_ndcg@k`, `cold_mrr`
- `zero_hr@k`, `zero_ndcg@k` (strict-cold bucket, count=0)
- `few_hr@k`, `few_ndcg@k` (long-tail bucket, count in [1,5])
