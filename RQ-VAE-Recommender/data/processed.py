...
class ItemData(Dataset):
    def __init__(
        ...
        **kwargs,
        ...
        external_ids = raw_data.data["item"].get("external_ids")
        if external_ids is None:
            external_ids = torch.arange(raw_data.data["item"]["x"].size(0))
        self.item_external_ids = external_ids[filt].long()
        self.item_text = raw_data.data["item"].get("text")
