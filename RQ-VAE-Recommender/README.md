...
For the shared `NewsRecommend` conda runtime used with HSTU integration, install:

```bash
pip install -r requirements-newsrecommend.txt
```
...
### ML-1M cold-start tokenizer runbook

Use the `NewsRecommend` conda env and the new ML-1M config:
...
`configs/rqvae_ml1m.gin` enables:
- MiniLM content encoder from local path (`/root/autodl-tmp/models/all-MiniLM-L6-v2`)
- EMA codebook updates with dead-code reset
- local CSV logging (`metrics.csv`) by default (`wandb_logging=False`, can be enabled)
- Bridge export artifact (`bridge_artifacts.pt`) with:
  - `item_id_to_codes`
  - `codebook_vectors`
  - optional `cold_item_mask`

If you run scripts directly (not through `make`), use:

```bash
conda run -n NewsRecommend env PYTHONPATH=. python train_rqvae.py configs/rqvae_ml1m.gin
```

For background runs with realtime logs:

```bash
mkdir -p logs
nohup bash -lc "cd /root/autodl-tmp/HSTU_RQVAE/RQ-VAE-Recommender && conda run -n NewsRecommend make train_rqvae config=configs/rqvae_ml1m.gin" \
  > logs/rqvae_$(date +%F_%H-%M-%S).log 2>&1 &
tail -f logs/rqvae_*.log
```
